  ...
  socket.onmessage = (event) => {
    ...
    switch (data.type) {
      ...
      case 'game_invite_declined':
        ...
        break;
      case 'guess_choice_made':
        if (currentModule) currentModule.handleChoiceMade(data);
        break;
      case 'guess_result':
        if (currentModule) currentModule.handleGuessResult(data);
        break;
      case 'battleship_placed':
        if (currentModule) currentModule.handleBattleshipPlaced(data);
        break;
      case 'battleship_start':
        if (currentModule) currentModule.handleBattleshipStart(data);
        break;
      case 'battleship_result':
        if (currentModule) currentModule.handleBattleshipResult(data);
        break;
    }
  };
...
function renderActiveGames(games) {
  ...
  const gameNames = { gomoku: '⚫ 五子棋', tictactoe: '⭕ 井字棋', rps: '✂️ 石头剪刀布', connect4: '🔴 四子棋', othello: '⚫ 黑白棋', guess: '🔢 猜数字', battleship: '🚢 海战棋' };
...
function showInviteModal(data) {
  ...
  const gameIcons = { gomoku: '⚫', tictactoe: '⭕', rps: '✂️', connect4: '🔴', othello: '⚫', guess: '🔢', battleship: '🚢' };
